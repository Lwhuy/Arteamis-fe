...
def test_migration_25_is_registered_in_both_lists():
    """Migration 25 must be appended to up and down lists (hard-coded, not auto-discovered)."""
    manager = AsyncMigrationManager()
    assert len(manager.up_migrations) == 25
    assert len(manager.down_migrations) == 25


def test_migration_25_defines_relates_edge():
    """Cleaned SQL for migration 25 defines the source->source `relates` edge."""
    up = AsyncMigration.from_file("open_notebook/database/migrations/25.surrealql")
    sql = up.sql
    ...
    down = AsyncMigration.from_file("open_notebook/database/migrations/25_down.surrealql")
