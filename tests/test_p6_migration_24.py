"""Migration 24 registration + content guards (P6 rollout: episode workspace scoping).
...
def test_migration_24_registered():
    ...
    assert "workspace" in mgr.up_migrations[23].sql
    assert "episode" in mgr.up_migrations[23].sql


def test_migration_24_up_adds_workspace_field_and_index():
    sql = (MIGRATIONS / "24.surrealql").read_text()
...
def test_migration_24_down_removes_field_and_index():
    sql = (MIGRATIONS / "24_down.surrealql").read_text()
...
def test_no_inline_comments_in_migration_24():
    ...
    for name in ("24.surrealql", "24_down.surrealql"):
