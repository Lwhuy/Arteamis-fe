...
Run Arteamis with a **separately installed Ollama** (not via Docker). This avoids Docker running the Ollama service while you use your own local Ollama installation.

## Prerequisites

1. **Docker Desktop** installed (for SurrealDB and Arteamis)
...
## Step 3: Connect Arteamis to Host Ollama (1 min)

When Arteamis runs inside Docker, it cannot reach `localhost:11434` on your host directly. Use the special hostname:

| Host OS | Ollama URL in Arteamis |
|---------|----------------------------|
...
## Step 7: Access Arteamis (instant)
...
- **Add more models**: Run `ollama pull <model>`, then re-discover from Arteamis
