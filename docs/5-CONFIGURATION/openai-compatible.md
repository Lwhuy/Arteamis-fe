...
Use any server that implements the OpenAI API format with Arteamis. This includes LM Studio, Text Generation WebUI, vLLM, and many others.
...
Arteamis can connect to any server using this format.
...
When Arteamis runs in Docker and your compatible server runs on the host, use the appropriate base URL when adding your credential in **Settings → API Keys**:
...
4. Update model name in Arteamis
```
